...
1. __NF_ACCEPT__ - принять пакет и не вызывать следующие коллбеки в хуке.
1. __NF_STOLEN__ - пакет **украден** - т.е. есть коллбек изъял пакет из дальнейшей обработки в сетевом стеке. Это используется, например, в коде сборки фрагментов в __conntrack__ и в реализации __SYN-прокси__. 
1. __NF_QUEUE__ - пакет направляется в __netfilter queue__, и будет обработан в юзерспейсе.
1. __NF_REPEAT__ - пропустить пакет через этот же коллбек ещё раз.
1. __NF_STOP__ - назначение этого результата почти такое же, как и __NF_ACCEPT__, но чуть сложнее, и оно используется в [потрохах](http://lxr.free-electrons.com/source/net/netfilter/nf_queue.c?v=4.7#L193) __netfilter_queue__, связанных с возвращением пакета обратно в тот же хук (т.н. reinjecting).
