...
Статья больше писалась для себя параллельно с расковыриванием кода ядра, для того, чтобы лучше усвоить полученные знания. Использовал ядро версии 4.7, на участки исходников которого и буду приводить ссылки. Старался писать интересно, добавляя подробности, почерпнутые из мейл-листов, которые иногда почитываю. Надеюсь, кому-нибудь это пригодится. В добрый путь!
...
Максимальное количество элементов в хеш-таблице вычисляется по формуле **HTSize>** * **max_factor**, где **max_factor** - максимальное количество элементов в цепочке в каждой ячейке. Это так называемый фактор хеш-таблицы. Если у нас в каждой цепочки хеш-таблицы будет много элементов, то, естественно, чтобы найти нужную запись в них, понадобится больше времени, так как мы не можем перейти сразу к нужному соединению в цепочке, так как местоположение этого соединения неизвестно (только значение хеша и сама информация о соединении), и вынуждены двигаться по цепочке с самого начала (обходить список), проверяя каждый элемент.
...
Из вышеописанного следует: первым делом увеличивайте __hashsize__(оно же __nf_conntrack_buckets__), а потом уже __nf_conntrack_max__, если нужно. Это позволит снизить потребление ЦПУ при очень интенсивном трафике, но чуть увеличит потребление памяти. Для более равномерного распределения соединения по ячейкам хеш-таблицы и уменьшения коллизий, количество ячеек можно делать нечётным, а ещё можно даже выбрать простое число (как писал Дональд Кнут).

Так же внутри коннтрака живёт дополнительная таблица под названием __expectations__. Как можно догадаться из названия, в ней хранятся шаблоны потоков, которые порождаются основными потоками. О ней мы поговорим чуть позже, когда коснёмся трассировки протоколов прикладного уровня. Архитектурна она устроена аналогично основной таблице потоков. Так же на неё, соответственно, распространяются те же самые правила по выбору и изменению размеров, что и на основную таблицу. Управляется через __/sys/module/nf_conntrack/parameters/expect_hashsize__ и __nf_conntrack_expect_max__.

Для того, чтобы просмотреть содержимое этих таблиц, можно использовать чтение из файлов псевдо-фс /proc/ (лучше не делать так - в мейл-листах упоминается состояние гонки, которое может возникнуть при этом), но лучше воспользоваться утилитой ```conntrack```, которая использует netlink-протокол для общения с ядром, а так же содержит встроенные средства фильтрации по различным параметрам, например, по протоколу и номерам портов. Подробности использования можно почитать в man'е. Сама утилита входит в пакет __conntrack__ (кто бы мог подумать!) или __conntrack-tools__. Можно собрать из исходников из [гит-репы](git://git.netfilter.org/conntrack-tools/). В мане к утилите __conntrack__ упоминаются ещё эфимерные таблицы __dying__ и __unconfirmed__ - о них мы поговорим в главе, посвящённой жизненному циклу потоков внутри коннтрака.
...
Пока про структуру, представляющую поток, рассказано достаточно. Теперь поговорим про то, как живёт представление о потоке, и как оно меняется по мере прохождения различных пакетов этого потока.

### Жизненный цикл потоков в коннтраке

Жизненный цикл потока неразрывно связан с состоянием потока, которое непрерывно меняется при поступлении (или непоступлении) трафика данного потока. Состояние потока (представлено полем ```status``` структуры ```nf_conn```) является битовым массивом длиной, соответствующем ```unsigned long```, который на различных архитектурах может разниться.

Теперь рассмотрим то, какие состояния может иметь поток в коннтраке. Описывается оно значением поля __status__, является целочисленным значением, и принимает следующие значения (названия констант определены [тут](http://lxr.free-electrons.com/source/include/uapi/linux/netfilter/nf_conntrack_common.h?v=4.7#L39)):

*__IPS_EXPECTED__ - порождённое хелпером соединение.

*__IPS_SEEN_REPLY__ - через поток прошёл ответный пакет.

*__IPS_ASSURED__ - защищённые потоки.

*__IPS_CONFIRMED__ - новый поток, пакет которого успешно прошёл через все цепочки, и был подтверждён.

*__IPS_SRC_NAT__ - для потока применяется трансляция адреса/портов источника.

*__IPS_DST_NAT__ - для потока применяется трансляция адреса/портов назначения.

*__IPS_SEQ_ADJUST__ - для потока применяется трансляция sequence-номеров. Применяется главным образом в __syn-прокси__.

*__IPS_SRC_NAT_DONE__ - трансляция адреса/портов источника завершена.

*__IPS_DST_NAT_DONE__ - трансляция адреса/портов назначения завершена.

*__IPS_DYING__ - информация о потоке больше не нужна. Это когда мы давно не видели пакетов этого потока (по таймауту), либо последний пакет потока прошёл через нас (например, соединение закрылось по __tcp-fin__ или через __tcp-reset__).

*__IPS_FIXED_TIMEOUT__ - у потока фиксированное время жизни.

*__IPS_TEMPLATE__ - эфимерный поток, являющийся шаблоном информации о потоках.

*__IPS_UNTRACKED__ - не отслеживать поток.

*__IPS_HELPER__ - к потоку прикреплён хелпер для трасировки прикладного протокола.
